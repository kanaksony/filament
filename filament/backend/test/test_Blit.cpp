...
static constexpr int kSrcTexWidth = 256;
static constexpr int kSrcTexHeight = 256;
static constexpr auto kSrcTexFormat = filament::backend::TextureFormat::RGBA8;
...
static void createBitmap(DriverApi& dapi, Handle<HwTexture> texture, int level, float3 color) {
    auto cb = [](void* buffer, size_t size, void* user) { free(buffer); };
    const int width = kSrcTexWidth >> level;
    const int height = kSrcTexHeight >> level;
...
TEST_F(BackendTest, ColorBlit) {
    ...
    createBitmap(api, srcTexture, 0, float3(0.5, 0, 0));
    createBitmap(api, srcTexture, 1, float3(0, 0, 0.5));
    ...
    // Grab a screenshot.
    getDriverApi().beginFrame(0, 0);
    dumpScreenshot(api, dstRenderTargets[0], "ColorBlit.png");
    getDriverApi().commit(swapChain);
    getDriverApi().endFrame(0);
